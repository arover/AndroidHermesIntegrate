...
protected:
  ...
  clonePropNameID(const Runtime::PointerValue *pv) override {
  ...
  ArrayBuffer
  createArrayBuffer(std::shared_ptr<MutableBuffer> buffer) override {
  ...
  getHeapInfo(bool includeExpensive) override {
...
template <typename T, typename U = void> struct BeforeCaller {
  static void before(T &) {}
};

template <typename T, typename U = void> struct AfterCaller {
  static void after(T &) {}
};
...
template <typename T> struct BeforeCaller<T, decltype((void)&T::before)> {
  static void before(T &t) { t.before(); }
};

template <typename T> struct AfterCaller<T, decltype((void)&T::after)> {
...
// example.
template <typename... T> struct BeforeCaller<std::tuple<T...>> {
...
private:
  ...
  template <size_t N> static void all_before(std::tuple<T...> &) {}
};

template <typename... T> struct AfterCaller<std::tuple<T...>> {
...
private:
  ...
  template <size_t N> static void all_after(std::tuple<T...> &) {}
};
