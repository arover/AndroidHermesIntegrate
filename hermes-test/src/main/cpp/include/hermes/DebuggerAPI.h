...
struct HERMES_EXPORT EvalResult {
  ...
  EvalResult(::facebook::jsi::Value value, bool isException,
             ExceptionDetails exceptionDetails)
      : value(std::move(value)), isException(isException),
...
public:
  ...
  PauseReason getPauseReason() const { return pauseReason_; }

  /// \return the breakpoint if the PauseReason is Breakpoint, otherwise
  /// kInvalidBreakpoint.
  BreakpointID getBreakpoint() const { return breakpoint_; }
  ...
  const StackTrace &getStackTrace() const { return stackTrace_; }
  ...
  /// parameters.
  VariableInfo getVariableInfo(uint32_t frameIndex,
...
public:
  ...
  const ProgramState &getProgramState() const { return state_; }
...
private:
  friend std::unique_ptr<HermesRuntime>
  hermes::makeHermesRuntime(const ::hermes::vm::RuntimeConfig &);
  ...
  explicit Debugger(::facebook::hermes::HermesRuntime *runtime,
...
public:
  ...
  PauseReason getPauseReason() const { return PauseReason::Exception; }

  BreakpointID getBreakpoint() const { return 0; }

  EvalResult getEvalResult() const { return EvalResult(); }
...
public:
  ...
  static Command step(StepMode mode) { return Command(); }
  static Command continueExecution() { return Command(); }
  static Command eval(const String &src, uint32_t frameIndex) {
    return Command();
  }
...
public:
  ...
  void setIsDebuggerAttached(bool isAttached) {}
  void triggerAsyncPause(AsyncPauseKind kind) {}
  const ProgramState &getProgramState() const { return programState_; }
  String getSourceMappingUrl(uint32_t fileId) const { return ""; };
  std::vector<SourceLocation> getLoadedScripts() const { return {}; }
  StackTrace captureStackTrace() const { return StackTrace{}; }
  BreakpointID setBreakpoint(SourceLocation loc) { return 0; }
  void setBreakpointCondition(BreakpointID breakpoint,
  ...
  void setShouldPauseOnScriptLoad(bool flag) {}
  bool getShouldPauseOnScriptLoad() const { return false; }
  ::facebook::jsi::Value getThrownValue() {
