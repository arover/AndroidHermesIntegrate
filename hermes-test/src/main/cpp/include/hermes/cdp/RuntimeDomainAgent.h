...
public:
  RuntimeDomainAgent(int32_t executionContextID, HermesRuntime &runtime,
  ...
  void
  discardConsoleEntries(const m::runtime::DiscardConsoleEntriesRequest &req);
...
private:
  ...
  bool
  validateExecutionContextId(m::runtime::ExecutionContextId executionContextId,
                             long long commandId);

  std::optional<std::vector<m::runtime::PropertyDescriptor>>
  makePropsFromScope(std::pair<uint32_t, uint32_t> frameAndScopeIndex,
  ...
  makePropsFromValue(const jsi::Value &value, const std::string &objectGroup,
                     bool onlyOwnProperties, bool accessorPropertiesOnly,
                     const ObjectSerializationOptions &serializationOptions);
  std::vector<m::runtime::InternalPropertyDescriptor>
  makeInternalPropsFromValue(
      const jsi::Value &value, const std::string &objectGroup,
      const ObjectSerializationOptions &serializationOptions);
