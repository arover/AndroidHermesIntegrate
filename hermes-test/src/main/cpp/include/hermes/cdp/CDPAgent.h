...
struct HERMES_EXPORT State {
  ...
  inline operator bool() const { return privateState_ != nullptr; }

  /// Get the wrapped state.
  inline Private &operator*() { return *privateState_.get(); }
...
class HERMES_EXPORT CDPAgent {
  ...
  /// methods.
  CDPAgent(int32_t executionContextID, CDPDebugAPI &cdpDebugAPI,
           debugger::EnqueueRuntimeTaskFunc enqueueRuntimeTaskCallback,
           OutboundMessageFunc messageCallback, State state,
...
public:
  ...
  static std::unique_ptr<CDPAgent>
  create(int32_t executionContextID, CDPDebugAPI &cdpDebugAPI,
         debugger::EnqueueRuntimeTaskFunc enqueueRuntimeTaskCallback,
         OutboundMessageFunc messageCallback, State state = {});
