...
template <typename... Args> class SynchronizedCallback {
public:
...
class DomainAgent {
protected:
  DomainAgent(int32_t executionContextID,
              SynchronizedOutboundCallback messageCallback,
              std::shared_ptr<RemoteObjectsTable> objTable)
      : executionContextID_(executionContextID),
        messageCallback_(messageCallback), objTable_(objTable) {}
  virtual ~DomainAgent() {}

  /// Sends the provided string back to the debug client
  void sendToClient(const std::string &str) { messageCallback_(str); }
