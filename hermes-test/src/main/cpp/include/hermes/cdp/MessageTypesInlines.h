...
template <typename T>
typename std::enable_if<std::is_same<T, JSONObject *>::value,
                        std::unique_ptr<T>>::type
...
template <typename T>
typename std::enable_if<std::is_same<T, JSONValue *>::value,
                        std::unique_ptr<T>>::type
valueFromJson(JSONValue *v) {
...
bool assign(std::unique_ptr<T, std::function<void(D *)>> &lhs,
            const JSONObject *obj, const U &key) {
  JSONValue *v = obj->get(key);
  if (v != nullptr) {
    auto convertResult = valueFromJson<T>(v);
    if (convertResult) {
...
// Pass through JSONValues.
inline JSONValue *valueToJson(JSONValue *v, JSONFactory &factory) { return v; }
...
template <typename V>
void put(Properties &props, const std::string &key, const V &value,
         JSONFactory &factory) {
...
template <typename V>
void put(Properties &props, const std::string &key, const optional<V> &optValue,
         JSONFactory &factory) {
...
template <typename V>
void put(Properties &props, const std::string &key,
         const std::unique_ptr<V> &ptr, JSONFactory &factory) {
  if (ptr.get()) {
    JSONString *jsStr = factory.getString(key);
