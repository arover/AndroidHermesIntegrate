...
public:
  DebuggerDomainAgent(int32_t executionContextID, HermesRuntime &runtime,
  ...
  void
  setPauseOnExceptions(const m::debugger::SetPauseOnExceptionsRequest &req);
  ...
  void
  setBreakpointsActive(const m::debugger::SetBreakpointsActiveRequest &req);

private:
  /// Handle an event originating from the runtime.
  void handleDebuggerEvent(HermesRuntime &runtime,
                           debugger::AsyncDebuggerAPI &asyncDebugger,
  ...
  void
  sendScriptParsedNotificationToClient(const debugger::SourceLocation srcLoc);
  ...
  createHermesBreakpont(debugger::ScriptID scriptID,
                        const CDPBreakpointDescription &description);

  std::optional<HermesBreakpointLocation>
  applyBreakpoint(CDPBreakpoint &breakpoint, debugger::ScriptID scriptID);
