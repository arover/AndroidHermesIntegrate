...
class INSPECTOR_EXPORT CDPHandler {
  ...
  /// methods.
  CDPHandler(std::unique_ptr<RuntimeAdapter> adapter, const std::string &title,
             bool waitForDebugger, bool processConsoleAPI,
...
public:
  ...
  static std::shared_ptr<CDPHandler>
  create(std::unique_ptr<RuntimeAdapter> adapter, bool waitForDebugger = false,
         bool processConsoleAPI = true, std::shared_ptr<State> state = nullptr,
         const CDPHandlerSessionConfig &sessionConfig = {},
         std::optional<CDPHandlerExecutionContextDescription>
             executionContextDescription = std::nullopt);
  /// Temporarily kept to allow React Native build to still work
  static std::shared_ptr<CDPHandler>
  create(std::unique_ptr<RuntimeAdapter> adapter, const std::string &title,
         bool waitForDebugger = false, bool processConsoleAPI = true,
  ...
  bool registerCallbacks(CDPMessageCallbackFunction msgCallback,
...
struct INSPECTOR_EXPORT State {
  ...
  Private &get() { return *privateState_.get(); }
