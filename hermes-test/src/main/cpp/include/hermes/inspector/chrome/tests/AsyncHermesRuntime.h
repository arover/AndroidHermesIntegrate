...
public:
  ...
  std::shared_ptr<HermesRuntime> runtime() { return runtime_; }
   ...
   */
  void executeScriptAsync(const std::string &str,
  ...
  void executeScriptSync(
      const std::string &script, const std::string &url = kDefaultUrl,
      facebook::hermes::HermesRuntime::DebugFlags flags =
          facebook::hermes::HermesRuntime::DebugFlags{},
      std::chrono::milliseconds timeout = std::chrono::milliseconds(2500));
  ...
  void evaluateBytecodeAsync(const std::string &bytecode,
   ...
   */
  void
  wait(std::chrono::milliseconds timeout = std::chrono::milliseconds(2500));
...
private:
  jsi::Value shouldStop(jsi::Runtime &runtime, const jsi::Value &thisVal,
                        const jsi::Value *args, size_t count);

  jsi::Value storeValue(jsi::Runtime &runtime, const jsi::Value &thisVal,
                        const jsi::Value *args, size_t count);
...
struct SamplingProfilerRAII {
  ...
  ~SamplingProfilerRAII() { runtime_.unregisterForProfilingInExecutor(); }
