...
/// order to prevent callers from constructing an eval Command. The runner_eval
...
using DebuggerEventCallback =
    std::function<void(HermesRuntime &runtime, AsyncDebuggerAPI &asyncDebugger,
...
using EvalCompleteCallback = std::function<void(
...
public:
  ...
  /// \p resumeFromPaused.
  DebuggerEventCallbackID
  addDebuggerEventCallback_TS(DebuggerEventCallback callback);
  ...
  /// for the next action.
  bool evalWhilePaused(const std::string &expression, uint32_t frameIndex,
...
private:
  ...
  std::list<EventCallbackEntry>::iterator
      eventCallbackIterator_ TSA_GUARDED_BY(mutex_);
...
class HERMES_EXPORT
AsyncDebuggerAPI{public : static std::unique_ptr<AsyncDebuggerAPI> create(
    HermesRuntime & runtime){return nullptr;
} // namespace debugger
...
bool isWaitingForCommand() { return false; }

bool isPaused() { return false; }

bool resumeFromPaused(AsyncDebugCommand command) { return false; }
