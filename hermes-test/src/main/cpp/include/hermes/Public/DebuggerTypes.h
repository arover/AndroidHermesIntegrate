...
struct StackTrace {
  ...
  uint32_t callFrameCount() const { return frames_.size(); }
...
enum class PauseReason {
  ...
  EvalComplete,         /// An runner_eval() function finished.
};
...
struct LexicalInfo {
  ...
  ScopeDepth getScopesCount() const { return variableCountsByScope_.size(); }
