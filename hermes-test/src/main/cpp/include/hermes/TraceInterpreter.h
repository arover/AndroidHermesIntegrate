...
public:
  ...
  static std::string
  execAndGetStats(const std::string &traceFile,
...
private:
  TraceInterpreter(
      jsi::Runtime &rt, const ExecuteOptions &options, const SynthTrace &trace,
      std::map<::hermes::SHA1, std::shared_ptr<const jsi::Buffer>> bundles);

  static std::string
  exec(jsi::Runtime &rt, const ExecuteOptions &options, const SynthTrace &trace,
       std::map<::hermes::SHA1, std::shared_ptr<const jsi::Buffer>> bundles);

  static ::hermes::vm::RuntimeConfig
  merge(::hermes::vm::RuntimeConfig::Builder &,
        const ::hermes::vm::GCConfig::Builder &, const ExecuteOptions &, bool,
        bool);
  ...
  getSourceHashToBundleMap(
      std::vector<std::unique_ptr<llvh::MemoryBuffer>> &&codeBufs,
      const SynthTrace &trace, const ExecuteOptions &options,
      bool *codeIsMmapped = nullptr, bool *isBytecode = nullptr);

  jsi::Function
  createHostFunction(const SynthTrace::CreateHostFunctionRecord &rec,
  ...
  void addToObjectMap(SynthTrace::ObjectID valID, jsi::Value &&val,
                      uint64_t defIndex);

  /// Similar to addToObjectMap, but for PropNameIDs.
  void addToPropNameIDMap(SynthTrace::ObjectID id, jsi::PropNameID &&val,
                          uint64_t defIndex);
  ...
  /// useful validation.
  void ifObjectAddToObjectMap(SynthTrace::TraceValue traceValue,
                              const jsi::Value &val, uint64_t defIndex,
                              bool isThis = false);

  /// Same as above, except it avoids copies on temporary objects.
  void ifObjectAddToObjectMap(SynthTrace::TraceValue traceValue,
                              jsi::Value &&val, uint64_t defIndex,
                              bool isThis = false);
  ...
  crashOnException(const std::exception &e,
                   ::hermes::OptValue<uint64_t> globalRecordNum);

  void assertMatch(const SynthTrace::TraceValue &traceValue,
