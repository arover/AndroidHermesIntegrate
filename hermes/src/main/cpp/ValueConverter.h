...
static Value valueFromJObject(Runtime &runtime, alias_ref<JObject> jObject) {
  if (jObject->isInstanceOf(JBoolean::javaClassStatic())) {
    ...
    JMap<jobject, jobject> *jmap =
        static_cast<JMap<jobject, jobject> *>(jObject.operator->());
    Object ret(runtime);
    for (const auto &entry : *jmap) {
      if (entry.first) {
        ret.setProperty(runtime,
                        PropNameID::forUtf8(runtime, entry.first->toString()),
...
static local_ref<JObject> jObjectFromValue(Runtime &runtime,
                                           const Value &value) {
  ...
  if (value.isUndefined() || value.isNull()) {
    ...
    if (obj.isArray(runtime)) {
      ...
      static auto listCtor =
          JArrayList<JObject>::javaClassStatic()
              ->getConstructor<JArrayList<JObject>::javaobject()>();
      auto jlist = JArrayList<JObject>::javaClassStatic()->newObject(listCtor);
      auto addMethod = JArrayList<JObject>::javaClassStatic()
                           ->getMethod<jboolean(local_ref<JObject>)>("add");
      ...
      static auto mapCtor =
          JHashMap<JString, JObject>::javaClassStatic()
              ->getConstructor<JHashMap<JString, JObject>::javaobject()>();
      auto map =
          JHashMap<JString, JObject>::javaClassStatic()->newObject(mapCtor);
      auto putMethod =
          JHashMap<JString, JObject>::javaClassStatic()
